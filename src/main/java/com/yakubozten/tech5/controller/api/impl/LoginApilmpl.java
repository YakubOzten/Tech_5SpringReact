...
import com.yakubozten.tech5.controller.api.ILoginApi;
...
public class LoginApilmpl implements ILoginApi {
    //Injection
    ...
    public ResponseEntity<?> login( @RequestHeader(name = "Authorization",required = false) String authorization) {
        ...
        if (findRegisterEntityByEmail==null){
            ApiResult apiResultNotFound=ApiResult.builder()
                    .path("/login/api/v1.0.0/authentication")
                    .status(404)
                    .message("Kullanıcı bulunamadı Kayıt olman gerekiyor")
                    .build();
    ...
    public ResponseEntity<?> logout(HttpServletRequest request, HttpServletResponse response) {
        ...
        if (authentication != null ){
            ...
            return ResponseEntity.ok(ApiResult.builder().path("/logout")
                    .status(200).message("Sistemden Çıkış Yapıldı.").build());
        }
