...
// onSubmitSearch
    ...
    const registerCreateSubmit = async (event) => {
        ...
        // API
        try {
            const response = await RegisterServicesApi.registerApiCreate(registerCreateObject);
            if (response.status == 200) {
                // Spinner pasif et
                setSpinner(false);// MultipleRequest (Aktif)
...
// Spinner
    ...
    // input değerlerinin içi dolu olduğunda aktif olsun
    const loginObjectisNull =
        registerNickName &&
        registerName &&
        registerSurname &&
        registerEmail &&
